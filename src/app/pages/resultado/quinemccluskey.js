...
export class QuineMcCluskey {
        ...
        for (const subset of powerset) {
            ...
            for (const implicant of subset) {
                for (const value of implicant.getValues()) {
                    if (!this.valueIn(value, tempValues) && this.valueIn(value, values)) {
        ...
        // Try comparing the rest
                ...
                for (const term1 of group1) {
                    for (const term2 of group2) {
                        ...
                        if (term3 !== null) {
                            term1.use();
                            term2.use();
                            if (!this.valueIn(term3, newGroups[compare])) {
            ...
            for (const group of groups) {
                for (const term of group) {
                    if (!term.isUsed() && !this.valueIn(term, unused)) {
                        unused.push(term);
                    }
                }
            }
    ...
    solve() {
        ...
        for (let i = 0; i < this.values.length; i++) {
            ...
            for (const minterm of primeImplicants) {
                if (this.valueIn(value, minterm.getValues())) {
                    uses += 1;
                    last = minterm;
                }
            }
            if (uses == 1 && !this.valueIn(last, essentialPrimeImplicants)) {
                for (const v of last.getValues()) {
                    if (!this.valueIn(v, this.dontCares)) {
        ...
        for (const implicant of primeImplicants) {
            if (!this.valueIn(implicant, essentialPrimeImplicants)) {

                // Check if the current implicant only consists of dont cares
                let add = false;
                for (const value of implicant.getValues()) {
                    if (!this.valueIn(value, this.dontCares)) {
                        add = true;
                        break;
                    }
                }
    ...
    getFunctionFormat() {
        ...
        let result = this.getFunction(this.isMaxterm);
        ...
        if (this.isMaxterm) {
            ...
            for (let g of groups) {
                ...
                for (let or of g) {
                    or = or.split(" ");
                    let variable = this.removeSpaces(or);

                    if (variable[0] == "NOT") {
                        newGroup.push(variable[1] + "'");
                    } else {
                        newGroup.push(variable[0]);
                    }
                }
            ...
            for (let g of groups) {
                ...
                for (let and of g) {
                    and = and.split(" ");
                    let variable = this.removeSpaces(and);
